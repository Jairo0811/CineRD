...
const foto = (perfil) => perfil?.Foto || perfil?.FotoUrl || perfil?.Imagen || perfil?.ImagenUrl || perfil?.ProfilePath || null;
...
function DashboardTalento({ usuario }) {
  ...
  useEffect(() => {
    api.get("/verificacion/mi-perfil")
  ...
  return <div className="talent-dashboard">
    <section className="talent-hero">
      {foto(perfil) ? <img className="talent-avatar" src={foto(perfil)} alt={nombre}/> : <div className="talent-avatar-placeholder">🎭</div>}
    ...
    <section className="talent-dashboard-grid">
      ...
      <Link to="/actores" className="talent-action-card"><span className="talent-action-icon">🤝</span><div><h2>Directorio de talentos</h2><p>Descubre actores, directores, productores y otros profesionales dominicanos.</p></div></Link>
    </section>

    <section className="talent-profile-panel">
      <article className="talent-panel"><header className="talent-panel-header"><span>Ficha profesional</span><h2>Información registrada en CineRD</h2></header><div className="talent-facts">
        <div className="talent-fact"><small>Nombre completo</small><strong>{perfil?.NombreCompleto || usuario?.nombre || "—"}</strong></div>
        <div className="talent-fact"><small>Nombre artístico</small><strong>{perfil?.NombreArtistico || "—"}</strong></div>
        <div className="talent-fact"><small>Profesión</small><strong>{profesion}</strong></div>
        <div className="talent-fact"><small>Filmografía registrada</small><strong>{peliculas} producciones</strong></div>
      </div></article>
      <article className="talent-panel"><header className="talent-panel-header"><span>Confianza</span><h2>Identidad profesional protegida</h2></header><div className="talent-trust"><div className="talent-trust-badge"><span>🛡️</span><div><strong>Verificación activa</strong><p>Esta cuenta está vinculada a un único perfil artístico. Las modificaciones sensibles del catálogo continúan bajo control administrativo para proteger la integridad de CineRD.</p></div></div></div></article>
    </section>
  </div>;
}
