...
function AdminGaleria() {
  ...
  const guardar = async (e) => {
    ...
    try {
      ...
      console.error(e2);
      setError(e2.response?.data?.mensaje || "No fue posible guardar la imagen");
    } finally {
      setGuardando(false);
    }
  };
  ...
  const eliminar = async (id) => {
    ...
    try {
      ...
      console.error(e);
      setError(e.response?.data?.mensaje || "No fue posible eliminar la imagen");
    }
  };
  ...
  return (
    <div className="table-page-container">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">Administra fotografías de rodaje, material promocional, pósteres alternativos y archivos visuales de películas y talentos.</p>
      </header>
      ...
      <section className="card mb-4">
        <div className="card-body p-4">
          <h2 className="h5 mb-3">{editandoId ? "Editar imagen" : "Agregar imagen"}</h2>
          <form onSubmit={guardar} className="row g-3">
            <div className="col-12 col-md-3">
              <label className="form-label">Entidad</label>
              <select className="form-select" value={form.entidad} onChange={(e)=>cambiarEntidad(e.target.value)}>
            ...
            <div className="col-12 col-md-4">
              ...
              <select className="form-select" value={form.Tipo} onChange={(e)=>cambiar("Tipo",e.target.value)}>
            ...
            <div className="col-12 col-md-6">
              ...
              <input className="form-control" maxLength={180} value={form.Titulo} onChange={(e)=>cambiar("Titulo",e.target.value)} />
            </div>
            <div className="col-12 col-md-4">
            ...
            <div className="col-12 col-md-4">
              ...
              <input className="form-control" type="url" maxLength={500} value={form.FuenteUrl} onChange={(e)=>cambiar("FuenteUrl",e.target.value)} />
            </div>
            ...
            <div className="col-12 d-flex flex-wrap gap-3 align-items-center">
              <div className="form-check">
                ...
                <label className="form-check-label" htmlFor="galeriaDestacada">Imagen destacada</label>
              </div>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <span className="text-muted small">{itemsFiltrados.length} imágenes</span>
      </div>

      {cargando ? <div className="text-center py-5"><div className="spinner-border" /></div> : (
        <div className="row g-3">
          {itemsFiltrados.map((item)=><div className="col-12 col-sm-6 col-lg-4 col-xl-3" key={item.Id}>
            <article className="card h-100 overflow-hidden">
              <img src={resolverImagen(item.Archivo)} alt={item.Titulo || item.Pelicula || item.Talento || "Galería CineRD"} style={{width:"100%",aspectRatio:"4/3",objectFit:"cover"}} />
              <div className="card-body">
                <div className="d-flex justify-content-between gap-2 align-items-start mb-2">
                  <span className="badge bg-secondary">{TIPOS.find(([v])=>v===item.Tipo)?.[1] || item.Tipo}</span>
                  {item.EsDestacada && <span title="Destacada">⭐</span>}
                </div>
          ...
          {!itemsFiltrados.length && <div className="col-12"><div className="text-center text-muted py-5">Todavía no hay imágenes registradas.</div></div>}
        </div>
      )}
    </div>
  );
