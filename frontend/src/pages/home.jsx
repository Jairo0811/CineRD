...
const imagenActor = (a) => a?.Foto || a?.FotoUrl || a?.Imagen || a?.ImagenUrl || a?.ProfilePath || null;
const imagenPelicula = (p) => p?.Poster || p?.PosterUrl || p?.Imagen || p?.ImagenUrl || null;
...
function Home() {
  ...
  const topActores = useMemo(() => [...actores].sort((a,b) => Number(b.CantidadPeliculas || 0) - Number(a.CantidadPeliculas || 0)).slice(0,6), [actores]);
  ...
  return <div className="admin-dashboard">
    ...
    <section className="admin-dashboard-section-grid">
      <article className="admin-dashboard-panel"><header className="admin-dashboard-panel-header"><div><span>Ranking de talentos</span><h2>Mayor presencia en el catálogo</h2></div><Link to="/actores">Ver todos</Link></header><div className="admin-ranking">{topActores.map((a,i)=><Link to={`/actores/${a.Id}`} className="admin-ranking-item" key={a.Id}><span className="admin-ranking-position">{i+1}</span>{imagenActor(a)?<img className="admin-ranking-image" src={imagenActor(a)} alt=""/>:<span className="admin-ranking-placeholder">🎭</span>}<span className="admin-ranking-copy"><strong>{a.NombreCompleto}</strong><small>{a.Profesion||"Talento cinematográfico"}</small></span><span className="admin-ranking-total">{a.CantidadPeliculas||0}</span></Link>)}</div></article>
    ...
    <section className="admin-recent-grid">
      <article className="admin-dashboard-panel"><header className="admin-dashboard-panel-header"><div><span>Actividad reciente</span><h2>Últimas películas agregadas</h2></div><Link to="/peliculas">Catálogo</Link></header><div className="admin-recent-list">{ultimasPeliculas.map(p=><Link to={`/peliculas/${p.Id}`} className="admin-recent-item" key={p.Id}>{imagenPelicula(p)?<img className="admin-recent-thumb" src={imagenPelicula(p)} alt=""/>:<span className="admin-recent-placeholder">🎬</span>}<span className="admin-recent-copy"><strong>{p.Titulo}</strong><small>{p.Genero||"Producción dominicana"}</small></span></Link>)}</div></article>
      <article className="admin-dashboard-panel"><header className="admin-dashboard-panel-header"><div><span>Actividad reciente</span><h2>Últimos talentos agregados</h2></div><Link to="/actores">Talentos</Link></header><div className="admin-recent-list">{ultimosTalentos.map(a=><Link to={`/actores/${a.Id}`} className="admin-recent-item" key={a.Id}>{imagenActor(a)?<img className="admin-recent-thumb" src={imagenActor(a)} alt=""/>:<span className="admin-recent-placeholder">🎭</span>}<span className="admin-recent-copy"><strong>{a.NombreCompleto}</strong><small>{a.Profesion||"Talento cinematográfico"}</small></span></Link>)}</div></article>
    </section>
  </div>;
}
export default Home;