    ...
    <nav className="navbar navbar-expand-lg navbar-dark sticky-top cine-navbar cine-navbar-cinematic">
      <div className="container">
        ...
        <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#cineRdNavbar"
          aria-controls="cineRdNavbar" aria-expanded="false" aria-label={t("nav.toggleNavigation")}>
          <span className="navbar-toggler-icon" />
        </button>

        <div className="collapse navbar-collapse" id="cineRdNavbar">
          <ul className="navbar-nav me-auto mb-3 mb-lg-0">
            <li className="nav-item"><NavLink className={obtenerClaseNav} to="/" end onClick={cerrarMenuMovil}>{t("nav.home")}</NavLink></li>
            <li className="nav-item"><NavLink className={obtenerClaseNav} to="/peliculas" onClick={cerrarMenuMovil}>{t("nav.movies")}</NavLink></li>
            <li className="nav-item"><NavLink className={obtenerClaseNav} to="/actores" onClick={cerrarMenuMovil}>{t("nav.talents")}</NavLink></li>
            {usuario && <li className="nav-item"><NavLink className={obtenerClaseNav} to="/dashboard" onClick={cerrarMenuMovil}>{t("nav.space")}</NavLink></li>}
            {esUsuario && <li className="nav-item"><NavLink className={obtenerClaseNav} to="/verificar-perfil" onClick={cerrarMenuMovil}>{t("nav.verifyProfile")}</NavLink></li>}
            {esAdmin && <li className="nav-item"><NavLink className={obtenerClaseNav} to="/admin/verificaciones" onClick={cerrarMenuMovil}>{t("nav.verifications")}</NavLink></li>}
          </ul>
          ...
          <div className="cine-navbar-actions">
            ...
            {esAdmin && (
              <div className="cine-admin-quick-actions">
                <NavLink className="btn cine-btn-ghost btn-sm" to="/peliculas/nueva" onClick={cerrarMenuMovil}>{t("nav.newMovie")}</NavLink>
                <NavLink className="btn cine-btn-accent btn-sm" to="/actores/nuevo" onClick={cerrarMenuMovil}>{t("nav.newTalent")}</NavLink>
              </div>
            )}

            {!usuario ? (
              <>
                <NavLink className="btn cine-btn-ghost btn-sm" to="/login" onClick={cerrarMenuMovil}>{t("nav.login")}</NavLink>
                <NavLink className="btn cine-btn-accent btn-sm" to="/registro" onClick={cerrarMenuMovil}>{t("nav.register")}</NavLink>
              </>
            ) : (
              <div className="dropdown">
                ...
                <ul className="dropdown-menu dropdown-menu-end cine-account-menu">
                  <li><NavLink className="dropdown-item" to="/dashboard" onClick={cerrarMenuMovil}>{t("nav.dashboard")}</NavLink></li>
                  {esUsuario && <li><NavLink className="dropdown-item" to="/verificar-perfil" onClick={cerrarMenuMovil}>{t("nav.verifyProfile")}</NavLink></li>}
                  {esAdmin && <li><NavLink className="dropdown-item" to="/admin/verificaciones" onClick={cerrarMenuMovil}>{t("nav.manageVerifications")}</NavLink></li>}
                  <li><hr className="dropdown-divider" /></li>
                  <li><button type="button" className="dropdown-item text-danger" onClick={cerrarSesion}>{t("nav.logout")}</button></li>
                </ul>
              </div>
            )}
          </div>
